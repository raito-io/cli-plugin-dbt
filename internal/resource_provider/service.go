...
func (s *DbtService) RunDbt(ctx context.Context, dbtFile string) (uint32, uint32, uint32, uint32, error) {
	manifest, err := s.manifestParser.LoadManifest(dbtFile)
	if err != nil {
...
func (s *DbtService) loadAccessProvidersFromManifest(manifest *manifest.Manifest) (map[string]*sdkTypes.AccessProviderInput, map[string]*sdkTypes.AccessProviderInput, map[string]*sdkTypes.AccessProviderInput, error) {
